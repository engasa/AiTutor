...
- **API Client**: `app/lib/api.ts` wraps `fetch`, injects the `Authorization` header, performs JSON parsing, and forces logout on 401/403. It exposes high-level helpers for courses, modules, lessons, activities, templates, prompts, and submissions that mirror backend endpoints.
...
- **Lesson Authoring UX**: Instructor lesson builder supports MCQ and short-text activities, optional AI prompt assignment per activity, prompt creation with temperature/topP controls, and inline imports from templates or existing offerings.
...
- **Business Logic Helpers**:
  - `cloneTemplateIntoOffering`, `cloneTemplateLessonsIntoModule`, and `cloneLessonsFromOffering` replicate template content into live offerings.
...
- **Endpoint Highlights**:
  ...
  - Templates: `GET /api/templates`, `GET /api/templates/:id`, `POST /api/courses/:courseId/import` (with template/module/lesson import options).
  - Courses: `GET /api/courses` (role-aware), `POST /api/courses`, `PATCH /api/courses/:id`.
...
- **Prisma Schema**: Defines users (with `Role` enum), course templates/offerings, modules, lessons, activities, prompt templates, activity types, enrollments, instructor assignments, and submissions. Activity `config` JSON stores question text, options, answers, and hints, enabling flexible activity types.
...
- Builds rich course templates (Algorithms, Linear Algebra, Physics) with nested modules/lessons/activities.
- Clones templates into live course offerings, assigns instructors/enrollments, creates custom capstone content, copies lessons across offerings/templates, and populates sample submissions for analytics demos.
...
- **Cloning logic**: reuse `cloneTemplateIntoOffering`, `cloneTemplateLessonsIntoModule`, or `cloneLessonsFromOffering` when building import features; each already preserves nested activities/prompt assignments.
