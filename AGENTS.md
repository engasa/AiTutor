...
- `bun run build` — emit SSR bundles for client/server.
...
- Run `bun run typecheck` before PRs and add regression tests whenever auth, role gating, or cloning logic changes.
